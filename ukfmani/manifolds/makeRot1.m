...
m.log = @(X) so3log(unflatten(X)); % TODO
m.exp = @(x) flatten(so3exp(x)); % TODO 
m.delta = @(X,Y) so3log(unflatten(X)*unflatten(Y)');  % TODO
m.step = @(X,y) flatten(so3exp(y)*unflatten(X)); % TODO 
